...
void motorBody(){
    ...
    if(motor_body<=100){
      ...
      analogWrite(OUTPUT_1, map(motor_body, 110,0,0,255));
     
    }
    else if(motor_body>=150){
      digitalWrite(DIR_PIN_1, HIGH);
      Serial.print("     forwards    ");
      analogWrite(OUTPUT_1, map(motor_body, 150,255,0,255));
    }
    else{
      Serial.print("     STOP    ");
      digitalWrite(OUTPUT_1, LOW);
...
void motorSail(){
    ...
    if(motor_sail<=100){
      ...
      Serial.print("     backwards     ");
      analogWrite(OUTPUT_2, map(motor_sail, 110,0,0,255));
    }
    else if(motor_sail>=150){
      digitalWrite(DIR_PIN_2, HIGH);
      Serial.print("     forwards    ");
      analogWrite(OUTPUT_2, map(motor_sail, 150,255,0,255));
    }
    else{
      Serial.print("     STOP    ");
      digitalWrite(OUTPUT_2, LOW);
